...
public class PlayerCtrlTest {
    ...
    public void setup() {
        ...
        when(playerFactory.createComputerPlayer(anyString(), anyInt(), any(Player.class), any(StrategyCtrl.class))).thenReturn(computer);
        strategyCtrl = mock(StrategyCtrl.class);
        playerCtrl = new PlayerCtrlImpl(playerFactory, strategyCtrl);
    }
    ...
    public void shouldBeAbleToSetupAOnePlayerGame() {
        ...
        verify(playerFactory).createPlayer(constants.GAME_PIECE_ONE, constants.SIDE);
        verify(playerFactory).createComputerPlayer(constants.GAME_PIECE_TWO, constants.SIDE, player1, strategyCtrl);
    }
    ...
    public void shouldBeAbleToSpecifyComputerPlayerAsPlayerOne() {
        ...
        verify(playerFactory).createPlayer(constants.GAME_PIECE_TWO, constants.SIDE);
        verify(playerFactory).createComputerPlayer(constants.GAME_PIECE_ONE, constants.SIDE, player1, strategyCtrl);
    }
}